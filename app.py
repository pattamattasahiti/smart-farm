...
def predict_yield(N, P, K, temperature, humidity, ph, rainfall):
    ...
    w_N, w_P, w_K, w_T, w_H, w_pH, w_R = 0.2, 0.15, 0.15, 0.1, 0.1, 0.15, 0.15
    
    max_yield = 200 
    
  
    yield_percentage = ((w_N * N + w_P * P + w_K * K + w_T * temperature + 
                         w_H * humidity + w_pH * ph + w_R * rainfall) / max_yield) * 100
    
    return min(100, max(0, yield_percentage)) 
...
def main():
    ...
    ph = st.slider("Soil pH", min_value=0, max_value=14, step=1)  
    ...
    if st.button("Predict"):
        ...
        yield_percentage = predict_yield(N, P, K, temperature, humidity, ph, rainfall)
        st.success(f"Estimated Yield: {yield_percentage:.2f}%")
        if season:
